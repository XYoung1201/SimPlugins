...
private:
	...
	void *data;
	bool *data_lockable;
	double* data_index_min;
	double* data_index_interval;
	double*** data_temp;
	double **index_value;
	int total_num;
	int *data_index_num;
...
void Interpolytion::realse_data(void* index,int n){
	if(n != index_num)
...
void Interpolytion::mem_build(void*& index,int n){
	if(n==index_num){
...
void Interpolytion::config(int input_num, int output_num, double* minValue, double* maxValue, double* data_step) {
	index_num = input_num;
...
void Interpolytion::boundryLimit(bool* limit) {
	if (data_num == 0)
		return;
	for (int i = 0; i < data_num; i++)
...
void Interpolytion::read_file(string file_name){
	if (data_num == 0)
		return;
	ifstream file;
	file.open(file_name);
...
double* Interpolytion::interpolyte(double* data_input){
	memcpy_s(output,data_num*sizeof(double),poly(data,0,data_input),data_num*sizeof(double));
	return output;
}

double* Interpolytion::poly(void* index,int n,double* input_data){
	if(n==index_num)
		return (double*)index;
